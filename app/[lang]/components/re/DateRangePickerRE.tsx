...
import React, { useState, useEffect, useRef, FocusEvent, useCallback } from "react"
import { CalendarMonth as CalendarMonthIcon, House as HouseIcon, Close as CloseIcon } from "@mui/icons-material"
import clsx from "clsx"
import { Paper, Box } from "@mui/material"

// date-fns
import { format, addDays, addMonths, startOfToday, isBefore, isAfter, isSameDay, eachMonthOfInterval, startOfMonth } from "date-fns"
...
const DateRangePicker: React.FC<DateRangePickerProps> = ({
	...
	const handleSearch = () => {
		if (onSearch) {
			...
			sendGAEvent("event", "search-initiated", {
				event_category: "engagement",
				event_label: "user_submitted_booking_search",
	...
	return (
		<Paper
			...
			<Box sx={{ width: "100%" }}>
				...
				<div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-1 sm:p-2">
					{/* Logo and Title */}
					...
					<div className="flex items-center space-x-4">
						{/* <select
							...
							onChange={(e) =>
								handleLocaleChange(
									e.target.value as "en" | "pl" | "it"
								)
							}
							className={`border ${c.border} ${c.input} ${c.text} rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-400`}
						>
			...
			{isOpen && (
				<div
					...
					{/* Calendar Grid */}
					<div className="p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 auto-rows-min">
											...
											return (
												<button
													...
													className={clsx(
														...
														disabled ? c.calendarDayDisabled : c.calendarText,
														!disabled && c.calendarDayHover,
