...
import { searchAvailable, searchAvailableOneProperty } from "@/utilities/functions/nobeds"
...
const ReservationEngine: React.FC<ReservationEngineProps> = ({
	...
	useEffect(() => {
		if (reservationStep === "roomSelection" && roomsSelected && roomsSelected.length > 0) {
			if (apartments === 1 && roomsSelected.length === 1) {
				// If only 1 apartment is needed and 1 is selected, go to next step
				dispatch(setReservationStepAction("summary"))
			} else if (apartments > 1 && roomsSelected.length === apartments) {
				// If multiple apartments are needed and all are selected, go to next step
				dispatch(setReservationStepAction("summary"))
			}
		}
	}, [roomsSelected, apartments, reservationStep, dispatch])
	...
	useEffect(() => {
		const autoSelectProperty = async () => {
			...
			if (targetProperty) {
				...
				const canAccommodateGuests = guests <= (targetProperty.maxOccupancy || 0)

				if (!stayPeriodValid) {
					// Property doesn't meet stay period requirements
					const message = booking?.propertyNotAvailable
						? `${booking.propertyNotAvailable} ${t("minmaxNotValid")}`
						: `${propertyName || "The requested property"} ${t("minmaxNotValid")}. ${
								booking?.suggestAlternatives || "Here are other available properties:"
							}`
					...
					dispatch(setReservationStepAction("roomSelection")) // Allow user to select different property
					return
				}

				if (!canAccommodateGuests) {
					// Property can't accommodate the guests
					const message = booking?.propertyNotAvailable
						? `${booking.propertyNotAvailable} ${t("cantFit")}`
						: `${propertyName || "The requested property"} ${t("cantFit")}. ${
								booking?.suggestAlternatives || "Here are other available properties:"
							}`
				...
				const message = booking?.propertyNotAvailable
					? `${booking.propertyNotAvailable} ${booking.suggestAlternatives || "Here are other available properties:"}`
					: `The requested property ${propertyName || ""} is not available for your selected dates. Here are other available options:`
		...
		autoSelectProperty()
	}, [propertyId, availabilityFound, guests, propertyName, booking, dispatch, numberOfNights, t])
			...
			*/}
			{/* {availabilityFound &&
				...
				reservationStep === "roomSelection" && (
					<FilterButton dictionary={finalFilterDictionary} />
				)} */}

			<ThemeProvider theme={muiTheme}>
				<div
					className={`w-full min-h-full p-3 ${
					...
					<div className="my-4">{renderDatePicker()}</div>
								...
								<div ref={alternativeDatesRef} className="flex gap-3 overflow-x-auto pb-2 max-h-60 mt-5">
									{alternativeRanges
										...
										.map((range, index) => (
											<div
												key={index}
												className="flex-shrink-0 w-48 p-4 border-2 rounded-lg cursor-pointer hover:shadow-md transition-all duration-200 bg-white"
					...
					{reservationStep === "payment" && (
						<div ref={paymentRef} className="flex flex-col items-center mt-8 px-4">
							{/* Payment Header */}
											...
											return (
												<div className="p-6 text-center">
													<div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-4">
														<div className="text-blue-600 text-4xl mb-3">ℹ️</div>
														...
														<div className="bg-white rounded-lg p-4 border-2 border-blue-200">
															<Typography variant="h6" className="text-gray-900 font-bold">
																{t("totalAmount")}:{" "}
																<span className="text-blue-600">{(servicesState.totalPrice || 0).toFixed(2)} PLN</span>
															</Typography>
														</div>
													</div>
												</div>
												...
												<div className="p-6 text-center">
													<div className="bg-red-50 border border-red-200 rounded-xl p-6 mb-4">
														...
														<div className="bg-white rounded-lg p-4 border-2 border-red-200">
															<Typography variant="h6" className="text-gray-900 font-bold">
																{t("totalAmount")}: <span className="text-red-600">{totalPrice.toFixed(2)} PLN</span>
															</Typography>
														</div>
													</div>
												</div>
											...
											return (
												<div className="p-6 text-center">
													<div className="bg-red-50 border border-red-200 rounded-xl p-6 mb-4">
														...
														<div className="bg-white rounded-lg p-4 border-2 border-red-200">
															<Typography variant="h6" className="text-gray-900 font-bold">
																{t("totalAmount")}: <span className="text-red-600">{finalTotalPrice.toFixed(2)} PLN</span>
															</Typography>
														</div>
													</div>
												</div>
											...
											return (
												<div className="p-6">
													<div className="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-6">
														...
														<Typography variant="h6" className="text-amber-700 font-semibold text-center mb-3">
															{t("partialPayment")}
														</Typography>
														<Typography variant="body2" className="text-amber-600 text-center mb-4">
															{t("partialPaymentInfo")}
														</Typography>
													</div>
													...
													<div className="bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 rounded-xl p-6 mb-6">
														<Typography variant="h6" className="text-gray-800 font-semibold mb-4 text-center">
															📊 {t("payment_breakdown")}
														</Typography>
														<div className="space-y-3">
															...
															<div className="border-t-2 border-gray-300 pt-3 mt-3">
																<div className="flex justify-between items-center py-3 px-4 bg-blue-50 rounded-lg border-2 border-blue-200">
																	...
																	<span className="font-bold text-blue-600 text-xl">{finalTotalPrice.toFixed(2)} PLN</span>
																</div>
															</div>
														</div>
													</div>
			...
			reservationStep === "book" ||
			reservationStep === "payment" ? (
				<div className="fixed bottom-0 left-0 right-16 bg-transparent border-t border-gray-200 shadow-lg z-10">
					<div className="flex justify-center items-center py-4 px-6 space-x-4">
						{/* Previous/Back Button */}
						{(reservationStep === "summary" || reservationStep === "book" || reservationStep === "payment") && (
							<BackButton
