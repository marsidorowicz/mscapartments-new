...
import { Box, Container, IconButton, Paper, Typography, createTheme, ThemeProvider, Collapse, TextField, Button, Checkbox } from "@mui/material"
...
const AdditionalServices: React.FC<Props> = ({ arrivalDate, departureDate, guests, theme = "light", locale, offerData }) => {
	...
	useEffect(() => {
		...
		if (servicesState.reservations?.length > 0 && !arrivalDate && !departureDate) {
			setReservationsWithParking(servicesState.reservations)
			setTotalPrice(servicesState.totalPrice)
			processedRef.current = true
		} else if (arrivalDate && departureDate && roomsSelected.length > 0 && reservationsWithParking.length === 0) {
	...
	}, [roomsSelected, servicesState, arrivalDate, departureDate, guests, numberOfNights, calculateTotalPrice, reservationsWithParking.length])
						...
						<Box sx={{ display: "flex", gap: 3 }}>
							<label style={{ display: "flex", alignItems: "center", gap: "8px" }}>
								<input
									...
									onChange={(e) => setPaymentOption(e.target.value as "30")}
									style={{ margin: 0 }}
								/>
								<Typography>{t("thirty_percent_upfront")}</Typography>
							</label>
							<label style={{ display: "flex", alignItems: "center", gap: "8px" }}>
								<input
									type="radio"
									...
									onChange={(e) => setPaymentOption(e.target.value as "100")}
									style={{ margin: 0 }}
								/>
								<Typography>{t("hundred_percent_upfront")}</Typography>
							</label>
						</Box>
