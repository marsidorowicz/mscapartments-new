...
export async function POST(req: NextRequest) {
	try {
		...
		await sendMailSDC({
			to: "biuro@mountainapartments.pl",
			subject: `Zapytanie ze strony Mountain Apartments`,
			html: `<b>Imię i nazwisko:</b> ${name}<br/><b>Email:</b> ${email}<br/><b>Telefon:</b> ${
				phone || "-"
			}<br/><b>Wiadomość:</b><br/>${message}`,
		})
