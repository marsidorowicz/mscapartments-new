	...
	const fetchCacheEntries = async (roomId: string, rangeStart: Date, rangeEnd: Date) => {
		...
		try {
			...
			setCacheEntries((prev) => {
				...
				if (prev.length > 0 && prev[0].propertyId.toString() !== roomId.toString()) {
					return fetched
				}
							...
							<div key={idx} className="min-w-0">
								...
								<div className="grid grid-cols-7 gap-1">
									{days.map((date, i) => {
										...
										const isPastDate = isBefore(date, effectiveMinDate)
										const dateKey = format(date, "yyyy-MM-dd")
										const cacheEntry = cacheEntryMap.get(dateKey)
										const currQuantity = !isPastDate && cacheEntry?.quantity != null ? cacheEntry.quantity : 0

										const prevDateKey = format(addDays(date, -1), "yyyy-MM-dd")
										const prevCacheEntry = cacheEntryMap.get(prevDateKey)
										const prevQuantity = !isPastDate && prevCacheEntry?.quantity != null ? prevCacheEntry.quantity : 0

										const isSelectingCheckout = selectedStartDate && !selectedEndDate
										...
										if (isPastDate) {
											...
											if (isSameDay(date, selectedStartDate)) {
												...
												let isValid = true
												let cursor = date
												while (isBefore(cursor, selectedStartDate)) {
													const cKey = format(cursor, "yyyy-MM-dd")
													const cEntry = cacheEntryMap.get(cKey)
													if (!cEntry || cEntry.quantity === 0 || cEntry.quantity === null) {
														isValid = false
														break
													}
													cursor = addDays(cursor, 1)
											...
											isClickable = currQuantity > 0
										}

										const isDisabledForClick = !isClickable
										...
										const fullyBooked = currQuantity === 0 && prevQuantity === 0
										const checkoutOnly = currQuantity === 0 && prevQuantity > 0
										const checkinOnly = currQuantity > 0 && prevQuantity === 0

										return (
											<button
												key={i}
												disabled={isDisabledForClick && !isStart && !isEnd}
												onClick={() => {
													if (isDisabledForClick && !isStart && !isEnd) return
													handleDateClick(date)
												}}
												className={clsx(
													"p-2 rounded-md transition-colors duration-200 relative overflow-hidden before:absolute before:inset-0 before:-z-10",
													isStart || isEnd ? "text-white" : isDisabledForClick ? "text-gray-300" : c.calendarDay,
													isStart || isEnd
														? c.calendarDaySelected
														: inRange
															? c.calendarDayInRange
															: isPastDate
																? "before:bg-gray-100"
																: fullyBooked
																	? "before:bg-[repeating-linear-gradient(45deg,transparent,transparent_6px,#cc9678_6px,#cc9678_12px)] before:opacity-40"
																	: checkoutOnly
																		? "before:bg-[repeating-linear-gradient(45deg,transparent,transparent_6px,#cc9678_6px,#cc9678_12px)] before:[clip-path:polygon(100%_0,100%_100%,0_100%)] before:opacity-40"
																		: checkinOnly
														...
														<>
															...
															<span
																className={clsx(
																	...
																	isStart || isEnd ? "text-white" : currQuantity > 0 ? "text-green-600" : "text-gray-300",
																)}>
																PLN
															</span>
														</>
