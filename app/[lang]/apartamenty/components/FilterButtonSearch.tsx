	...
	useEffect(() => {
		...
		if (typeof window !== "undefined") {
			...
			if (savedLastMinuteOnly) {
				try {
					const parsed = JSON.parse(savedLastMinuteOnly)
...
export default function FilterButtonSearch({ dictionary, className }: FilterButtonProps) {
		...
		<>
			...
			<div className={`z-50 transition-all duration-300 `}>
				<button
					...
					className={`bg-[#cc9678] hover:bg-[#b8856a] text-white px-4 py-2 rounded-md shadow-sm focus:ring-2 focus:ring-blue-400 transition-all duration-300 relative ${className || ""}`}
					...
					<div
						...
						<div className="p-6">
							...
							<div className="flex justify-between items-center mb-6">
								...
								{getActiveFilterCount() > 0 && (
									<button onClick={unselectAll} className="px-4 py-2 text-[#cc9678] hover:text-[#b8856a] font-medium transition-colors">
							...
							<div className="space-y-6">
								...
								<div className="mb-6">
									...
									<label
										className={`flex items-center p-4 rounded-lg border cursor-pointer transition-all ${lastMinuteOnly ? "border-[#cc9678] bg-[#f5f0eb] text-[#cc9678]" : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}>
										<input type="checkbox" checked={lastMinuteOnly} onChange={toggleLastMinuteOnly} className="sr-only" />
										<div
											className={`w-4 h-4 rounded border mr-3 flex items-center justify-center flex-shrink-0 ${lastMinuteOnly ? "bg-[#cc9678] border-[#cc9678]" : "border-gray-300 bg-white"}`}>
											{lastMinuteOnly && (
												<svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
													<path
														fillRule="evenodd"
						...
						<div className="sticky bottom-0 bg-gray-50 px-6 py-4 rounded-b-2xl border-t border-gray-200">
							<button
								onClick={() => setIsDialogOpen(false)}
								className="w-full bg-gradient-to-r from-[#cc9678] via-[#b8856a] to-[#a3745c] hover:from-[#b8856a] hover:via-[#a3745c] hover:to-[#8e634e] text-white py-3 px-6 rounded-lg font-medium transition-all duration-300">
								{dictionary.close}
							</button>
						</div>
					</div>
